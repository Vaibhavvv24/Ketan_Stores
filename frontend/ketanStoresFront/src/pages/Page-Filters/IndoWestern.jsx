...
export default function IndoWestern() {
  ...
  const handleJacketsAndSuits = (e) => {
    ...
    if (colour === "" && !isNaN(size)) {
      fetch(`http://localhost:8080/other/INDO_WESTERN/filter/${size}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${jwt}`,
      ...
      <div className='grid lg:grid-cols-2 w-full gap-3 px-10 h-full sm:grid-cols-1'>
        {!loading &&
          ...
          data.map((item, index) => {
            console.log(item);
            return (
              ((search !== "" && item.typeName === "INDO_WESTERN" && item.name.toLowerCase().includes(search.toLowerCase())) || search === "") && <div key={index}>
                <ItemsPalette
                  filterItems={[item]}
                  Item={data[index]}
